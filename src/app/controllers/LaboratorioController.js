...
export default {
  async criarLaboratorio(request, response) {
    try {
      ...
      let resposta = null;
      // Verifica se o campo "nome" não é nulo (deve ser obrigatório)
      if (!nome || nome == undefined || nome == null || nome == "") {
        resposta = {
          status: 400,
          error: "O campo 'nome' deve ser obrigatório.",
        };
      }
      // Verifica se o campo "sigla" não é nulo (deve ser obrigatório)
      if (!sigla || sigla == undefined || sigla == null || sigla == "") {
        resposta = {
          status: 400,
          error: "O campo 'sigla' deve ser obrigatório.",
        };
      }
      if (!resposta) {
        resposta = await LaboratorioBusiness.criarLaboratorio({ nome, sigla });
      }
